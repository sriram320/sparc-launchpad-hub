      ...
      <section className="py-8 bg-card/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-6">
            ...
            {user?.role === 'host' && (
              <Link to="/host-dashboard/create-event" className="btn-mission">Create New Event</Link>
            )}
          </div>
            ...
            {filteredEvents.map((event) => (
              <div key={event.id} className="cosmic-card p-8 group">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Link to={`/event-booking/${event.id}`} className="flex-1">
                    <button className={`w-full ${event.isPaid ? 'btn-mission' : 'btn-ai'} flex items-center justify-center space-x-2`}>
